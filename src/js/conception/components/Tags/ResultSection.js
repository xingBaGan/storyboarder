...
import { Button, Input, message, Tooltip, Typography, Space, Tag } from "antd";
...
const ResultSection = ({ selectedTags = [], setSelectedTags, tagsData, promptText }) => {
  ...
  useEffect(() => {
    ...
    handleResultTextChange({ target: { value: promptText } })
  }, [promptText])
  ...
  const handleConstantText = useCallback(
    (constantText) => {
      ...
      const newSelectedTags = uniqueDisplayNames.map(displayName => {
        const { category, object, langName, displayName: foundDisplayName } = findTagData(displayName);
        return {
          category,
  ...
  const handleResultTextChange = useCallback(
    (e) => {
      ...
      const newSelectedTags = newText
        ...
        .filter(displayName => displayName && displayName.trim() !== "")
        .map(displayName => {
          const { category, object, langName } = findTagData(displayName);
          return { category, displayName, object, langName };
        });
      setSelectedTags(newSelectedTags);
    },
  ...
  const handleSuggestTagClick = (tag) => {
